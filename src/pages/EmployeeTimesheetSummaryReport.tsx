...
const rightAlignedColumns = new Set<ColumnKey>(['company_name', 'name', 'timesheet_status', 'timecard_status', 'remarks']);
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden bg-white">
      <style>{`@media print {
        ...
        #timesheet-summary-report th.print-align-right, #timesheet-summary-report td.print-align-right { text-align: right; }
      ...
      <div id="timesheet-summary-report" ref={reportRef} className="min-h-0 flex-1 overflow-auto p-3">
        ...
        {loading && !rows.length ? <div className="flex h-40 items-center justify-center gap-2 text-sm text-slate-400"><Loader2 className="h-4 w-4 animate-spin" />Loading report...</div> : <>
          <div className="overflow-auto rounded-lg border border-slate-200"><table className="w-full min-w-[1100px] border-collapse text-xs"><thead className="sticky top-0 z-10 bg-slate-800 text-left text-[10px] uppercase tracking-wide text-white"><tr>{columns.filter(({ key }) => visibleColumns[key]).map(({ key, label }) => <th key={key} className={`whitespace-nowrap px-3 py-1.5 font-medium print-col-${key} ${rightAlignedColumns.has(key) ? 'text-right print-align-right' : ''}`}><button type="button" onClick={() => toggleSort(key)} disabled={key === 'serialNumber'} className={`inline-flex items-center gap-1 disabled:cursor-default ${rightAlignedColumns.has(key) ? 'float-right' : ''}`}>{label}{sortIcon(key)}</button></th>)}</tr></thead><tbody>{filteredRows.length ? filteredRows.map((row, index) => <tr key={`${row.emp_id}-${row.date}-${index}`} className="border-t border-slate-100 even:bg-slate-50/60 hover:bg-teal-50/40">{columns.filter(({ key }) => visibleColumns[key]).map(({ key }) => <td key={key} className={`px-3 py-1 print-col-${key} ${rightAlignedColumns.has(key) ? 'text-right print-align-right' : ''} ${key === 'name' ? 'max-w-[180px] whitespace-normal break-words font-medium text-slate-700' : ''} ${key === 'remarks' ? 'max-w-[240px] whitespace-normal break-words' : ''} ${key.startsWith('display') ? 'tabular-nums' : ''}`}>{columnValue(row, key, index + 1)}</td>)}</tr>) : <tr><td colSpan={columns.filter(({ key }) => visibleColumns[key]).length} className="px-3 py-12 text-center text-slate-400">No records found</td></tr>}</tbody></table></div>
